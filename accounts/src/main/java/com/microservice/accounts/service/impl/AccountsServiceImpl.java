...
public class AccountsServiceImpl implements IAccountsService {
	...
	// as we have single constructor so no need to mention @Autowire
	...
	public void createAccount(CustomerDto customerDto) {
		...
		if (optionalCustomer.isPresent()) {
			throw new CustomerAlreadyExistsException(
					"Customer already registered with given mobileNumber " + customerDto.getMobileNumber()); // throwing
																												// custom
			// exception if mobile number already exists
		}

		// customer.setCreatedAt(LocalDateTime.now());
		// customer.setCreatedBy("Anonymous");

		Customer savedCustomer = customerRepository.save(customer); // saving customer records into database
		// it will populate the customer id
		// we need to store it in Customer object

		// now we want to create a new bank account for this customer
